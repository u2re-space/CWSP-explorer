...
import { En as __vitePreload, gn as ref, i as StorageKeys, jn as removeAdopted, mn as observe, nn as H, o as setString } from "../com/app.js";
import { f as readCwspSku, i as apkManifestForSku, l as isWebHubSurface, r as androidPackageForSku, s as inferCwspSkuFromLocation } from "../shells/boot-history-base.js";
...
var contributionVisible = (contribution, ctx) => {
	...
	if (contribution.id === "apk-update" && ctx.surface === "environment" && !isNativeApkHost()) return false;
...
var createSettingsView = (opts) => {
	...
	const apkUpdateTarget = () => {
		...
		const sku = resolveSettingsAreaNavMode() !== "none" && section !== "hub" ? skuForHubSettingsSection(section) : readCwspSku() || "launcher";
		return {
			sku,
	...
	Promise.resolve(loadSettingsForView()).then((s) => {
		...
		if (isCapacitorNative()) __vitePreload(() => import("../shells/boot-index.js").then((n) => n.Ht).then((m) => m.invokeCwsNative("app:info", apkUpdateTarget())), __vite__mapDeps([3,2,1,4,5,6,7]), import.meta.url).then((result) => {
