...
var persistProcessIngressNativeSnapshot = async (settings) => {
	...
	try {
		const { invokeCwsNative } = await __vitePreload(async () => {
			const { invokeCwsNative } = await import("./cws-bridge.js").then((n) => n.t);
...
var readCrxControlSessionTokenAsync = async () => {
	...
	try {
		return await (await __vitePreload(() => import("./crx-control-session2.js"), __vite__mapDeps([7,8]), import.meta.url)).getCrxControlSessionToken() || "";
	} catch {
		return "";
	}
};
var webnativeControl = async (path, init) => {
	try {
		...
		if ((res.status === 401 || res.status === 403) && (pageIsPublicHttps || pageIsChromeExtension)) try {
			...
			if (pageIsChromeExtension) __vitePreload(() => import("./crx-control-session2.js").then((m) => m.clearCrxControlSession()), __vite__mapDeps([7,8]), import.meta.url).catch(() => void 0);
			globalThis.dispatchEvent(new CustomEvent("cwsp-control-unauthorized", { detail: {
				status: res.status,
				path
			} }));
...
var loadLureFs = () => {
	...
	if (!lureFsPromise) lureFsPromise = __vitePreload(() => import("../vendor/jsox.js").then((n) => n.t).then((m) => ({
		getDirectoryHandle: m.getDirectoryHandle,
		readFile: m.readFile
	})), __vite__mapDeps([9,1,8,10]), import.meta.url);
...
//#endregion
export { shouldAnnotateCoordinatorPayload as a, loadSettings as c, DEFAULT_SETTINGS as d, annotateCoordinatorPayload as i, shouldDeferCrxHubSocketBootstrap as l, inferWireDedupeCategory as n, annotatePacketWireTime64 as o, packetWireDedupeGuard as r, ensureCapacitorCwspSettingsSeeded as s, annotatePacketWireHash as t, setAirpadCredentialInvalidator as u };
