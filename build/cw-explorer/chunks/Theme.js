...
var extractFromImage = async (src) => {
	try {
		...
		}, __vite__mapDeps([6,1,7,8,9]), import.meta.url);
	...
	(async () => {
		try {
			const { rehydrateAdoptedStyleSheets } = await __vitePreload(async () => {
				const { rehydrateAdoptedStyleSheets } = await import("../vendor/jsox.js").then((n) => n.t);
				return { rehydrateAdoptedStyleSheets };
			}, __vite__mapDeps([7,1,8,9]), import.meta.url);
