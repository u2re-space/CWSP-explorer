...
import { a as installNativeShowSaveFilePicker, b as writeNativeStorageFile, c as nativeUriFromSaveHandle, o as isNativeStorageAvailable, r as createNativeStorageDocument, v as toNativeStorageVirtualPath, x as writeNativeStorageUri } from "../com/app5.js";
				...
				if (isVirtualFsPath(src) || /^\/assets(?:\/|$)/i.test(src) || /^\/(?:sdcard|saf)(?:\/|$)/i.test(src)) (async () => {
					if (/^\/(?:sdcard|saf)(?:\/|$)/i.test(src)) try {
						const { ensureNativeStorageProvide } = await __vitePreload(async () => {
							const { ensureNativeStorageProvide } = await import("../com/app5.js").then((n) => n._);
					...
					if (!file && /^\/(?:sdcard|saf)(?:\/|$)/i.test(src)) try {
						const { readNativeStorageFile } = await __vitePreload(async () => {
							const { readNativeStorageFile } = await import("../com/app5.js").then((n) => n._);
		...
		async openMarkdownFromUrl(source, filename) {
			...
			try {
				...
				if (/^\/(?:sdcard|saf)(?:\/|$)/i.test(normalizedSource)) try {
					const bridge = await __vitePreload(() => import("../com/app5.js").then((n) => n._), __vite__mapDeps([2,1,3]), import.meta.url);
