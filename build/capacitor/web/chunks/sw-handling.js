...
import { An as __vitePreload, Et as bindDirectoryForLaunchedFiles, Gt as parseDataUrl, Ut as isBase64Like, dn as initClipboardReceiver, un as copy } from "../com/app.js";
...
var routeToTransferView = async (shareData, source, hint, pending = false) => {
	...
	try {
		...
		const { rememberOpenPolicyFromSettings } = await __vitePreload(async () => {
			const { rememberOpenPolicyFromSettings } = await import("../shells/boot-index.js").then((n) => n._t);
