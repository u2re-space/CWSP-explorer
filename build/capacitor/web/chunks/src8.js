...
import { n as apkManifestForSku, r as readCwspSku } from "../assets/index-DrCWYGbn.js";
...
* INVARIANT: Markdown / AI live on the document SKU; CWSP Control lives on transfer.
...
var pruneBuiltInSettingsTabs = (root, profile) => {
	const hidden = profile === "cwsp-mobile" ? CWSP_MOBILE_HIDDEN_BUILTIN_TABS : profile === "extension" ? EXTENSION_HIDDEN_BUILTIN_TABS : profile === "markdown" ? MARKDOWN_HIDDEN_BUILTIN_TABS : profile === "environment" ? ENVIRONMENT_HIDDEN_BUILTIN_TABS : profile === "explorer" ? EXPLORER_HIDDEN_BUILTIN_TABS : null;
	if (!hidden) return;
...
var contributionVisible = (contribution, ctx) => {
	...
	if (contribution.id === "cwsp") {
		...
		if (sku === "launcher" || sku === "explorer" || sku === "document") return false;
	}
	return true;
};
